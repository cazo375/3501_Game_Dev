...
namespace Level_Space {
	...
	void Level::createLevel (Ogre::SceneManager* scene_manager) {
		switch (levelNum) {
		case 0: 
			planets.push_back(Planet_Space::Planet(scene_manager, VENUS)); 
			enemies.push_back(Enemy_Space::Enemy (scene_manager, Ogre::Vector3 (0.0f, 0.0f, 750.0f)));
			break;
		case 1:
			planets.push_back(Planet_Space::Planet(scene_manager, EARTH));
			break;
		} 
	}
